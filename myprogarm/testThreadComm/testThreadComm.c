...
{  
    ...
    //pthread_setspecific(key,(void *)tid);
    sleep(10);
	if(GLO_NULL == (msgPtr = malloc(12)))
	{
	    printf("child2: failed to malloc memory with length %d,  returns\n",12);
		return;
	}
    
	msgList.msgPtr = msgPtr;
	msgList.msgLen = 12; 
	
	*((unsigned int *) msgPtr) = 0xABCDEF;
	*((unsigned int *) msgPtr+1) = 0xFECDAB;
